...
export default function MyEvents() {
  ...
  const [activeTab, setActiveTab] = useState('myEvents'); // 'myEvents' or 'bookedEvents'
  ...
  const fetchBookedEvents = async () => {
    try {
      ...
      const res = await fetch(`/api/events/artist/booked?artistId=${artistId}`);
      const data = await res.json();

      if (data.success) {
        setBookedEvents(data.registrations || []);
      }
    } catch (error) {
          ...
          <div className="flex gap-4">
            <div className="bg-white rounded-xl p-1 border border-gray-200 shadow-sm flex">
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium text-sm transition-all ${activeTab === 'myEvents'
                  ? 'bg-[#171C3C] text-white shadow-md'
                  : 'text-gray-600 hover:text-[#171C3C] hover:bg-gray-50'
                  }`}
              >
                My Events
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium text-sm transition-all ${activeTab === 'bookedEvents'
                  ? 'bg-[#171C3C] text-white shadow-md'
                  : 'text-gray-600 hover:text-[#171C3C] hover:bg-gray-50'
                  }`}
              >
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200 text-gray-500 text-sm">
                      <th className="p-4 font-medium">Attendee Details</th>
                      <th className="p-4 font-medium">Event</th>
                      <th className="p-4 font-medium">Tickets</th>
                      <th className="p-4 font-medium">Amount</th>
                      <th className="p-4 font-medium">Status</th>
                      <th className="p-4 font-medium">Date Booked</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {bookedEvents.map((booking) => (
                      <tr key={booking._id} className="hover:bg-gray-50 transition-colors">
                        <td className="p-4">
                          <p className="font-semibold text-[#171C3C]">{booking.name}</p>
                        ...
                        <td className="p-4">
                          {booking.eventId ? (
                            <>
                              <p className="font-medium text-[#171C3C]">{booking.eventId.title}</p>
                            ...
                            <span className="text-gray-400 italic">Event Deleted</span>
                          )}
                        </td>
                        <td className="p-4">
                          <div className="inline-flex items-center justify-center bg-blue-50 text-blue-700 font-medium px-2.5 py-1 rounded-md text-sm">
                            {booking.tickets}
                          </div>
                        </td>
                        <td className="p-4 font-medium text-[#171C3C]">
                          ₹{booking.totalAmount}
                        </td>
                        <td className="p-4">
                          <span className={`px-3 py-1 text-xs font-semibold rounded-full ${booking.paymentStatus === 'paid' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                            {booking.paymentStatus.charAt(0).toUpperCase() + booking.paymentStatus.slice(1)}
                          </span>
                        </td>
                        <td className="p-4 text-sm text-gray-500">
                          {new Date(booking.createdAt).toLocaleString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                        </td>
                      </tr>
                    ))}
                  </tbody>

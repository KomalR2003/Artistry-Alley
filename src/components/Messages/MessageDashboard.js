...
export default function MessageDashboard() {
  ...
  const forceRefresh = async () => {
    ...
    if (activeConversation && !activeConversation.isNew) {
      try {
        ...
        const data = await res.json();
        if (data.success) {
          // If message length is different, auto scroll over 
          setMessages(prev => {
             // Only update if there's a change or read receipts changed
  ...
  // Safe polling effect that doesn't trigger infinite renders
  useEffect(() => {
    if (!currentUser) return;
  ...
  const openNewChatModal = async () => {
    ...
    try {
      // Fetch users to chat with based on role
      const targetRoles = currentUser?.role === "admin" ? "artist,user" : "artist";
      const res = await fetch(`/api/users?role=${targetRoles}&excludeId=${currentUser._id}`);
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <div className="w-full h-full bg-[#f8f9fa] flex items-center justify-center px-4 md:px-8 lg:px-8 py-3" style={{ minHeight: "calc(100vh - 80px)" }}>
      <div className="w-full max-w-[1800px] flex bg-white rounded-3xl shadow-2xl border border-gray-300 overflow-hidden" style={{ height: "calc(100vh - 160px)", minHeight: "580px" }}>
        <div className="w-[320px] md:w-[380px] shrink-0 border-r-2 border-gray-200 flex flex-col bg-gray-50/50 shadow-[4px_0_15px_-3px_rgba(0,0,0,0.05)] z-10">
          ...
          <div>
            <h2 className="text-2xl font-semibold text-[#171C3C]">Chats</h2>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-1">
                       <h3 className={`font-semibold truncate ${isActive ? 'text-[#171C3C]' : 'text-gray-700 group-hover:text-[#171C3C]'}`}>{otherUser.username}</h3>
      ...
      <div className="flex-1 flex flex-col bg-white relative">
        {activeConversation ? (
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        <div>
                          <h2 className="font-semibold text-lg text-[#171C3C]">{otherUser?.username}</h2>
                  ...
                  return (
                    <div key={idx} className={`flex w-full ${isSentByMe ? 'justify-end' : 'justify-start'} ${isPrevSame ? 'mt-1' : 'mt-4'}`}>
                      <div className={`flex items-end gap-2 max-w-[75%] ${isSentByMe ? 'flex-row-reverse' : 'flex-row'}`}>
                        ...
                        {/* Bubble */}
                        <div className={`px-4 py-2 relative flex flex-col min-w-[80px]
                          ${isSentByMe 
                             ? 'bg-[#171C3C] text-white rounded-2xl rounded-br-sm shadow-sm'
                             : 'bg-[#f0f2f5] text-[#171C3C] rounded-2xl rounded-bl-sm shadow-sm'}`}>
                          <div className="flex flex-col">
                            <p className="text-[14.5px] font-medium leading-relaxed pb-2 pr-10">{msg.content}</p>
                            
                            {/* Timestamp always visible inside bubble */}
                            <div className="absolute bottom-1 right-2 flex items-center gap-1">
                              <p className={`text-[10px] font-bold whitespace-nowrap ${isSentByMe ? 'text-white/60' : 'text-gray-500'}`}>
                                {new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                              </p>
                              {isSentByMe && (
                                <CheckCheck size={14} className={msg.isRead ? "text-[#34B7F1]" : "text-white/60"} />
                              )}
                            </div>
                          </div>
                        </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-gray-400 relative z-10">
            ...
            <h2 className="text-2xl font-semibold text-[#171C3C] mb-3">Your Messages</h2>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[80vh] animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h3 className="font-semibold text-xl text-[#171C3C]">New Chat</h3>
              <button 
            ...
            <div className="p-4 border-b border-gray-100">
              <div className="relative">
                ...
                <input 
                  type="text" 
                  placeholder={currentUser?.role === "admin" ? "Search artists and users..." : "Search artists..."} 
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {isFetchingArtists ? (
                ...
                <div className="py-12 text-center text-gray-500 text-sm">No users found to chat with.</div>
              ) : (
